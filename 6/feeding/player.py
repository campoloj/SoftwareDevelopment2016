...
class Player(object):
    ...
    def next_feeding(cls, player, food_available, list_of_players):
        ...
        fat_tissue_species = [species for species in hungry_species if "fat-tissue" in species.trait_names()]
        if fat_tissue_species:
            return cls.feed_fatty(fat_tissue_species, food_available)

        hungry_herbivores = cls.find_hungry_herbs(hungry_species, hungry_carnivores)
        if hungry_herbivores:
            return cls.feed_herbivores(hungry_herbivores)
